...
class TechCompanyRecognizer(object):
    ...
    ._.has_tech_org and ._.is_tech_org is set on the Doc/Span and Token
    respectively."""
    name = 'tech_companies'  # component name/ pipeline name, will show up in the pipeline
    def __init__(self, nlp, companies=tuple(), label='ORG'):
        ...
        self.matcher.add('TECH_ORGS', None, *patterns)  # define matcher to match against patterns, which are comapnies
        # Register attribute on the Token. We'll be overwriting this based on
            # the matches, so we're only setting a default value, not a getter.
            # the attribute is defined below in the __call__
        Token.set_extension('is_tech_org', default=False)
        # Register attributes on Doc and Span via a getter that checks if one of
            # the contained tokens is set to is_tech_org == True.
            # the registered extension functions are defined below
...
nlp = English()  # start with blank pipeline with no model or pre-defined pipeline loaded
companies = ['Alphabet Inc.', 'Google', 'Netflix', 'Apple']  # etc.
component = TechCompanyRecognizer(nlp, companies)  # initialise component
nlp.add_pipe(component, last=True)  # add last to the pipeline
doc = nlp(text)
