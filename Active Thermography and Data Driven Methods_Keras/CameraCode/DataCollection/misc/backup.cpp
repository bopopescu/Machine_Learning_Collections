                ...
                if ( lType == PvPayloadTypeImage )
                {
                    ...
                    if (lBuffer->GetBlockID()%2==0) {
                        ...
                        if (server > 0)
                        {
                            ...
                            // Write File Attempt 1
                            // lBufferWriter.Store(lBuffer,"data.bin",PvBufferFormatRaw);




                            unsigned char* data = (unsigned char*) lBuffer->GetDataPointer();
                            uint32_t size = lBuffer->GetSize();
                            cout << endl << "Packet Size: " << size << endl;

                            // Write File Attempt 2
                            // ofstream myFile ("data.bin", ios::out | ios::binary);
                            // myFile.write ( (char*)data, 82944*sizeof(uint16_t) );
                            // myFile.flush();
                            // myFile.close();


                            // cout << endl << "Data is " << size << endl;
                            // cout << endl << size ;
                            // cout << " bytes sent from Server. " << endl;

                            // send(server, data, size, 0); // Sending full data
                            send(server, data, 2, 0); // Just for signaling purpose
                            ...
                            else
                            {
                                ...
                                strcat(response_buffer, schar);
                                strcat(response_buffer,".bin");
                                lBufferWriter.Store(lBuffer,response_buffer,PvBufferFormatRaw);
