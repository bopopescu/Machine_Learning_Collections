...
class RESTCountriesComponent(object):
    ...
    def __init__(self, nlp, label='GPE'):
        ...
        self.label = nlp.vocab.strings[label]  # get entity label ID, here nlp.vocab.strings['GPE'] is 382
        ...
        # Register attribute on the Token.
            # We'll be overwriting this based on
            # the matches, so we're only setting a default value, not a getter.
            # If no default value is set, it defaults to None.
        try:
            # The following is to register the attributes using __call__ funciton below
            ...
            pass  # in case if the set_extension is added already
    ...
    def __call__(self, doc):
        ...
        # Exampe output, doc = 'Some text about Colombia and the Czech Republic'
        # self.matcher(doc) = (2988880774438019688, 3, 4), where, 3,4 are start and end
            # of the locations of token that matches the *pattern that are countries
            # here, the words are Columbia and Czech Republic
        spans = []  # keep the spans for later so we can merge them afterwards
        for _, start, end in matches:
            # Generate Span representing the entity & set label
            entity = Span(doc, start, end, label=self.label)  # example, entity='Columbia'
            spans.append(entity)
            # Set custom attribute
                # on each token of the entity
        ...
        for span in spans:
            # Iterate over all spans and merge them into one token. This is done
                ...
                # indices!
            span.merge()
        return doc  # don't forget to return the Doc! here, the doc.ents are changed through __call__
