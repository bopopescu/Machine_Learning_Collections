...
def can_be_sentence_start(token):
    if token.i == 0:
        ...
        return True
    # punctuation:  the marks, such as period, comma,
        # and parentheses, used in writing to separate
        # sentences and their elements and to clarify meaning.
    elif token.nbor(-1).is_punct: # nbor(-1) means if previous position is
        return True
...
"""
Download the en_core_web_lg module from
$ python -m spacy download en_core_web_lg
"""

nlp = None  # clear the nlp model in case if it's loaded
nlp = spacy.load('en_core_web_lg')
raw_text = "Been here and I'm loving it."
doc = nlp(raw_text)
sentences = [sent.string.strip() for sent in doc.sents]
print(sentences)
# ['Been here', "and I'm loving it."]
