...
################ First Round of Regression for Test Train Split Propensity #############
traindf <- unique(full[, "transactiondate", with = F])
traindf[, trnascationYear := year(transactiondate)]
full <- merge(full, traindf, by = "transactiondate")
setkey(full, rowidx)
...
predictors_ <- intersect(setdiff(predictors, c(month_vars, non_predictors)),
                            colnames(full_small))
predictors_ <- c(predictors_, "transactionMonth")
...
for (i in seq(1, n_folds)) {
    trainidx <- which(full_small$split=="train" & full_small$foldid != i)
    train_dm_reg <- lgb.Dataset(
                    data = full_mm[trainidx, ],
                    label = full_small[trainidx, logerror],
                    weight = rep(1, length(trainidx))
                )
    valididx <- which(full_small$split=="train" & full_small$foldid == i)
    valid_dm_reg <- lgb.Dataset(
                    data = full_mm[valididx, ],
                    label = full_small[valididx, logerror],
                    weight = rep(1, length(valididx))
                )
    testidx <- which(full_small$split=="test")
    test_dm_reg <- lgb.Dataset(
                    data = full_mm[testidx, ]
                )
    param <- list(objective = "regression",
                    metric="l1",
                    num_leaves = 8, # 2**8 -1, interaction depth=5
                    learning_rate = 0.1,
                    boost_from_average = TRUE,
                    boosting_type = 'dart')  #gbdt is another choice
    set.seed(10011)
    ...
    lgb1_reg <- lgb.train(params = param,
                    ...
                    verbose = 1)
    gc()
    if (!"logerror_pred_v1" %in% colnames(full_small)) {full_small[, logerror_pred_v1 := 0.0]}
    valid_pred_ <- predict(lgb1_reg, full_mm[valididx, ])
    full_small[valididx, logerror_pred_v1 := valid_pred_]
    test_pred_ <- predict(lgb1_reg, full_mm[testidx, ])
    full_small[testidx, logerror_pred_v1 := logerror_pred_v1 + test_pred_]
    rm(train_dm_reg, valid_dm_reg); gc();
}

full_small[testidx, logerror_pred_v1 := logerror_pred_v1 / n_folds]


## Test Split Propensity
var_importance <- lgb.importance(lgb1_reg)
idx_ <- which(full_small[, split=="test" & parcelid %in% unique(full_small[split=="train", parcelid])])
full_small2 <- full_small[-idx_]
setkey(full_small2, rowidx)
full_mm <- data.matrix(full_small2[, c(var_importance$Feature[1], "logerror_pred_v1"), with = F]) * 1.0  # convert integer to float
n_folds <- 5
set.seed(10011)
full_small2[, random := runif(nrow(full_small2))]
full_small2[, foldid := ceiling(random / (1/n_folds))]
full_small2[, random := NULL]

scale_pos_weight <- nrow(full_small2[split=="train"])/nrow(full_small2[split=="test"])

for (i in seq(1, n_folds)) {
    train_dm_reg <- xgb.DMatrix(
                    data = full_mm[full_small2$foldid != i, ],
                    label = full_small2[foldid != i, 1*(split == "test")],
                    missing = NA
                )
    ...
    # print(paste0("Best GBM iteration is ", best.iter))
    valid_pred_ <- predict(xgb0_reg, full_mm[full_small2$foldid == i, ])
    full_small2[foldid == i, test_split_propensity_by_lgb := valid_pred_]
    rm(train_dm_reg, valid_dm_reg); gc();
}



full <- merge(full, full_small2[split=='train', c("rowidx", "test_split_propensity_by_lgb"), with = F],
                by = "rowidx", all.x=T, all.y=F)
full[is.na(test_split_propensity_by_lgb), test_split_propensity_by_lgb := 0.5]
full[, rec_weight_bylgb := test_split_propensity_by_lgb / (1-test_split_propensity_by_lgb)]
...
for (i in seq(1, n_folds)) {
    ...
    train_dm_reg <- lgb.Dataset(
                    ...
                    weight = full[trainidx, rec_weight_bylgb]
                )
    valididx <- which(full$split=="train" & full$foldid == i)
    valid_dm_reg <- lgb.Dataset(
                    data = full_mm[valididx, ],
                    label = full[valididx, pmin(0.382,pmax(-0.3425, logerror))],
                    weight = full[valididx, rec_weight_bylgb]
                )
    testidx <- which(full$split=="test")
    test_dm_reg <- lgb.Dataset(
                    data = full_mm[testidx, ]
                )
    param <- list(objective = "regression",
    ...
    # print(paste0("Best GBM iteration is ", best.iter))
    valid_pred_ <- predict(lgb1_reg, full_mm[valididx, ])
    full[valididx, logerror_pred_bylgb := valid_pred_]
    test_pred_ <- predict(lgb1_reg, full_mm[testidx, ])
    full[testidx, logerror_pred_bylgb := logerror_pred_bylgb + test_pred_]
