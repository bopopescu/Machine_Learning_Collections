...
# Get Full_Train only for Quantile Regression, no need of following new
# numerical variables
...
full_train_h2o_gbm <- h2o.gbm(x = predictors_prev, y = response, training_frame = full_train_h2o,
...
full_train_h2o_gbm_0.05 <- h2o.gbm(x = predictors_prev, y = response, training_frame = full_train_h2o,
                        #validation_frame = valid,
                        #keep_cross_validation_predictions= TRUE,
                        nfolds = 20,
                        distribution = 'quantile',
                        ...
                        col_sample_rate = 0.6,
                        sample_rate = 0.8,
                        ntrees = 71, max_depth = 5,
                        seed = 1234)

full_train_h2o_gbm_0.95 <- h2o.gbm(x = predictors_prev, y = response, training_frame = full_train_h2o,
                        #validation_frame = valid,
                        #keep_cross_validation_predictions= TRUE,
                        nfolds = 20,
                        distribution = 'quantile',
                        ...
                        col_sample_rate = 0.6,
                        sample_rate = 0.8,
                        ntrees = 40, max_depth = 5,
                        seed = 1234)

# print the mse for validation set
# print(h2o.mae(full_train_h2o_gbm, valid = TRUE))

predictions_mean <- as.vector(h2o.predict(full_train_h2o_gbm, full_train_h2o))
predictions_0.05 <- as.vector(h2o.predict(full_train_h2o_gbm_0.05, full_train_h2o))
predictions_0.95 <- as.vector(h2o.predict(full_train_h2o_gbm_0.95, full_train_h2o))
response_actual <- full[split=='train', get(response)]

x_ <- sum((response_actual > predictions_0.95) | (response_actual < predictions_0.05))
print(paste0("There are ", x_, " records out of ", length(response_actual)," records are outliers"))
# "There are 8634 records out of 90275 records are outliers"
#x=c(4,1,3,2)
#x2= sort(x)
#x[match(x=x2, table=x)]
## Plot confidence intervals
order_ <- match(x=sort(response_actual), table=response_actual)
dat <- cbind(t(t(response_actual[order_])), t(t(predictions_0.05[order_])), t(t(predictions_0.95[order_])))
...
######################################################################################################
rowidx_to_throwout <- full[split=='train'][which((response_actual > predictions_0.95) | (response_actual < predictions_0.05))][, rowidx]
...
summary(response_actual[!((response_actual > predictions_0.95) | (response_actual < predictions_0.05))])
