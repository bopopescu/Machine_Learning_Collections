...
class LSTMClassifier(nn.Module):
    ...
    def forward(self, x):
        ...
        # y_scores = F.log_softmax(y, dim=1) # return probability using log_softmax, dim=1 means sum is 1 for column (like axis=1)
...
for i in allids:
    ...
    for j in range(0, len(df_) - N_TIME_STEPS, step):
        ...
        labels.append(encode_label2(label))
    del df_
